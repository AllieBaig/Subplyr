...
export const Group = ({ title, icon: Icon, color, children, isExpanded, onToggle }: any) => {
  return (
    <div className="paper-card overflow-hidden mb-6 flex flex-col">
      <div className="bg-transparent">
        <button 
          onClick={onToggle}
          className="w-full flex items-center gap-4 text-left p-6 hover:bg-secondary-system-background/50 transition-colors paper-button"
        >
          <div className={`w-12 h-12 rounded-2xl ${color} flex-shrink-0 flex items-center justify-center shadow-sm`}>
            <Icon size={24} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="serif-title text-lg text-system-label tracking-tight">{title}</h3>
      ...
      <div className="p-5 flex items-center justify-between">
        <div className="flex items-center gap-4 min-w-0">
          <div 
            ...
            className={`w-10 h-10 ${isOpen ? 'bg-secondary-system-background' : 'bg-secondary-system-background/50'} rounded-2xl flex-shrink-0 flex items-center justify-center ${color} transition-all cursor-pointer paper-button`}
          >
            <Icon size={20} />
          </div>
        ...
        {onToggle !== undefined && (
          <button 
            ...
            className={`flex-shrink-0 w-10 h-6 rounded-full relative transition-colors paper-button ${isEnabled ? 'bg-apple-blue' : 'bg-system-tertiary-label'}`}
          >
